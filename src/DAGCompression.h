...
struct FPool
{
	...
	TStaticArray<T, EMemoryType::GPU> ShrinkArray(TDynamicArray<T, EMemoryType::GPU>& Array)
	{
		...
		{
			return TStaticArray<T, EMemoryType::GPU>(Array.GetData(), Array.Num());
		}
		else
		{
...
inline void CheckDag(const TStaticArray<uint32, EMemoryType::GPU>& Dag, uint32 Index, uint32 Level)
{
	ZoneScoped;
...
inline TStaticArray<uint32, EMemoryType::GPU> CreateDAG(const TStaticArray<uint64, EMemoryType::GPU>& InFragments, const int32 Depth)
{
	ZoneScoped;
